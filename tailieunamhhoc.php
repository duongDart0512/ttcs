...
$sql_count = "SELECT COUNT(*) as total FROM tailieu where namhocid = $idnamhoc";
...
where tailieu.namhocid = $idnamhoc order by tailieuname
...
<section id="courses" class="padding-medium" style = "margin-top:-50px;">
    ...
    <ul class="pagination justify-content-center">
        ...
        <?php if ($page > 1): ?>
            <li class="page-item">
                <a class="page-link" href="?id=<?= $idnamhoc ?>&page=<?= $page - 1 ?>">Previous</a>
            </li>
        <?php else: ?>
        ...
        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                <a class="page-link" href="?id=<?= $idnamhoc ?>&page=<?= $i ?>"><?= $i ?></a>
            </li>
        <?php endfor; ?>
        ...
        <?php if ($page < $total_pages): ?>
            <li class="page-item">
                <a class="page-link" href="?id=<?= $idnamhoc ?>&page=<?= $page + 1 ?>">Next</a>
            </li>
        <?php else: ?>
            <li class="page-item disabled">
