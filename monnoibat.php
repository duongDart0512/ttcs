...
$tenmonhoc = urldecode($_GET['tenonhoc']);
...
$sql_count = "SELECT COUNT(*) as total FROM tailieu where tenonhoc like'%$tenmonhoc%'";
...
where tailieu.tenonhoc = '$tenmonhoc' order by tailieuname
...
<div class= "bgtext" style = " margin-top:10px;height : 100px; max -width : 100vw; background: #4CC082; 
    display: flex; align-items: center; justify-content: center">
      <!-- <img src="images/bgtext.jpg" alt="" style = "height : 150px; width : 100vw" > -->
       <p style = "font-size : 30px; font-weight: bold">Tài liệu</p>
    </div>
<section id="courses" class="padding-medium" style = "margin-top:-50px;">
    ...
    <ul class="pagination justify-content-center">
        ...
        <?php if ($page > 1): ?>
            <li class="page-item">
                <a class="page-link" href="?tenonhoc=<?= $tenmonhoc ?>&page=<?= $page - 1 ?>">Previous</a>
            </li>
        <?php else: ?>
        ...
        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                <a class="page-link" href="?tenonhoc=<?= $tenmonhoc ?>&page=<?= $i ?>"><?= $i ?></a>
            </li>
        <?php endfor; ?>
        ...
        <?php if ($page < $total_pages): ?>
            <li class="page-item">
                <a class="page-link" href="?tenonhoc=<?= $tenmonhoc ?>&page=<?= $page + 1 ?>">Next</a>
            </li>
        <?php else: ?>
            <li class="page-item disabled">
